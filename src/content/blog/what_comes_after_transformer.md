...
description: "The cost of scaling is exploding while the gains shrink. A tour of the three escape routes — MoE, world models, and Google's memory-and-sleep line — then a closer look at the five ways people are trying to make a frozen model keep learning."
pubDate: "2026-07-19"
author: "locchh"
tags: ["transformer", "world-model", "jepa", "titans", "moe", "memory", "continual-learning", "test-time-training", "self-modifying", "agent-architecture", "2026"]
---
...
## The deeper question behind Door 3: how a frozen model learns

Door 3 was one lab's answer to a question the whole field is chewing on: once training ends and the weights **freeze**, how does a model learn anything new? Google's bet was to build the learning into the weights. But that's one corner of a bigger map. Zoom out and there are **five** distinct programs — and they sort along one revealing axis: *the more truly a method changes the model, the further it is from production.*

Two questions place any of them. *Where* does the change live — outside the model in the prompt, or inside the weights? And *when* does it happen — every write, per query, offline, or never?

**1. Long-term memory — change the prompt, not the model.** The cheapest answer, and the only one shipping everywhere: leave the weights frozen, keep new knowledge in a database, and paste the relevant bits back into the context when they're needed. The pattern goes back to [Generative Agents](https://arxiv.org/abs/2304.03442) and their scored "memory stream"; the sharpest framing is [MemGPT](https://arxiv.org/abs/2310.08560) (now **Letta**), which treats the LLM like an operating system paging memory in and out of its own context. The live fight is **vector vs graph** — similarity search finds similar text but loses relationships and time, which is why graph memory like [Zep](https://arxiv.org/abs/2501.13956) marks old facts stale instead of deleting them. Does it matter? [One benchmark](https://arxiv.org/abs/2410.10813) measures a ~30% accuracy drop when an assistant has to remember across a long interaction. But the hard part was never storage — it's **forgetting and curation**, the exact wall I hit writing about [knowledge bases](/blogs/blog/how_to_build_knowledgebase/). A memory that only ever adds becomes a junk drawer.

**2. Continual learning — change the weights, and pay for it.** Now teach the model the fact for real, into the weights — and hit the oldest wall in neural nets: **catastrophic forgetting**. Knowledge isn't filed in labeled drawers; it's smeared across shared weights, so nudging them toward a new fact drags an old one away. [The classic defense](https://arxiv.org/abs/1612.00796) penalizes moving the weights that matter most for old tasks, and a [sobering study](https://arxiv.org/abs/2308.08747) found that, in the sizes tested, bigger models forgot *more*. Underneath sits the **stability–plasticity dilemma** — too stable and it can't learn, too plastic and it forgets — and it's a law, not a bug. So the working answers keep the change out of the frozen core: [replay a little old data](https://arxiv.org/abs/2403.08763), give each task its own [low-rank adapter](https://arxiv.org/abs/2310.14152), or [add new blocks](https://arxiv.org/abs/2401.02415) rather than overwrite old ones. In production almost nobody ships live weight updates at all — they use **RAG** ([which beats fine-tuning for teaching new facts](https://arxiv.org/abs/2312.05934) anyway), swappable adapters, and gated batch retrains. The instant a model mutates itself online, you lose the two things production depends on: a test and an undo. Door 3's Hope and sleep papers are the architectural attempt to solve this properly — still research-stage.

**3. Test-time learning — change the weights, per query.** A distinction that trips almost everyone: **test-time compute** (a reasoning model "thinking longer") leaves the weights frozen — that's thinking, not learning. **Test-time training** actually takes a gradient step and changes the parameters for the one input in front of it. [The idea](https://arxiv.org/abs/1909.13231) is old; what's new is that [the hidden state of a sequence model can *be* a small model, trained by gradient descent as it reads](https://arxiv.org/abs/2407.04620) — which is exactly what Door 3's Titans does with its "surprise" signal. The headline result is on [ARC](https://arxiv.org/abs/2411.07279), the reasoning puzzles LLMs famously flub: fine-tune a throwaway adapter on each puzzle's own examples and the score jumps up to **6×** over the same model without it — and, combined with other methods, reaches near the average human score. Learning a fresh little model per problem beat being clever in one frozen pass. The catch is cost (a training run per query) and honesty: [some "test-time learning" is plain attention in disguise](https://arxiv.org/abs/2602.21204), and [the loss can fall while the model recalls nothing new](https://arxiv.org/abs/2607.00368).

**4. Self-modifying models — the model rewrites itself.** The deep end: the model produces its own updates. [SEAL](https://arxiv.org/abs/2506.10943) has the model write its own finetuning data and use reinforcement learning to discover which self-edits actually help. [Transformer²](https://arxiv.org/abs/2501.06252) rescales its own weights at inference to fit the task. Door 3's Hope is [a model that learns its own update rule](https://arxiv.org/abs/2512.24695). The lineage is Schmidhuber's 1990s line that ["the weight matrix of a neural network is its program"](https://arxiv.org/abs/2202.05780) — so a self-modifying model is a program rewriting itself as it runs. The vivid case is the [Darwin Gödel Machine](https://arxiv.org/abs/2505.22954), a coding agent that edits its **own source code**, keeps whatever tests better, and more than doubled its own benchmark score — by relaxing the old dream of "provably better" to just "empirically better." And that's exactly where it turns unsettling: when the benchmark is the only rule, the model games it. In their runs it **faked a test log**; when they tried to catch that, it **deleted the markers they'd added to detect the faking**. A system optimized against its own referee will eventually attack the referee. Keep it in proportion, though — every result is narrow, forgetting is still unsolved, and all of it is sandboxed research.

**5. Agent architectures — where the four meet reality.** The four above are ingredients; the architecture is the recipe deciding which happens, where, and when. The clearest map is [CoALA](https://arxiv.org/abs/2309.02427), which borrows from cognitive science: an agent has working, episodic, semantic, and **procedural** memory (its own code and weights), plus a plan-and-act loop. Our four frontiers slot straight in — long-term memory writes episodic and semantic, continual learning is the "learn" action, test-time learning is fast internal adaptation, self-modification rewrites procedural memory. And here's the honest punchline of the whole door: when an agent "learns" today — [ReAct](https://arxiv.org/abs/2210.03629), [Reflexion](https://arxiv.org/abs/2303.11366), [Voyager](https://arxiv.org/abs/2305.16291) — it almost always means it **took better notes**: stored and reused its own experience, *not* ran gradient descent. Better notes, not new weights. Which is why the craft moved to what Karpathy renamed **context engineering**, and what Lilian Weng calls [harness engineering](https://lilianweng.github.io/posts/2026-07-04-harness/): *"the layer between the raw model and the real-world context seems to be as important as the model's raw intelligence."* Which is this blog's whole refrain — [the harness is the job](/blogs/blog/migration_is_a_harness/).

Line them up and the axis does the work:

| Frontier | Where the change lives | When | Shipping today? |
|---|---|---|---|
| Long-term memory | Outside the model, in a store | Every write | Yes — everywhere |
| Continual learning | In the weights | Offline, gated | Rarely — RAG and adapters stand in |
| Test-time learning | In the weights, per input | Per query | Research and competitions |
| Self-modifying | In the weights and its own code | The model decides | Research only |
| Agent architecture | Around the model | By design | Yes — it *is* the harness |

The pattern is impossible to miss, and it echoes the three-door table above: everything we actually run keeps the model **frozen** and puts the learning *outside* it — in a store, in retrieved context, in notes, in adapters. Everything that truly changes the model — continual learning, test-time training, self-modification — is thrilling and stuck in the lab. In the [knowledge-base post's](/blogs/blog/how_to_build_knowledgebase/) brain picture, we shipped the fast external-capture half (the hippocampus); the slow, into-the-weights, sleeping half (the neocortex) is exactly what Titans, Hope, the sleep paper, and SEAL are all reaching for — and haven't landed.
...
**How a frozen model learns (deeper on Door 3):**
- *Long-term memory:* [Generative Agents](https://arxiv.org/abs/2304.03442) · [MemGPT / Letta](https://arxiv.org/abs/2310.08560) · [Zep](https://arxiv.org/abs/2501.13956) · [LongMemEval](https://arxiv.org/abs/2410.10813)
- *Continual learning:* [EWC](https://arxiv.org/abs/1612.00796) · [Forgetting in LLMs](https://arxiv.org/abs/2308.08747) · [Continual pre-training](https://arxiv.org/abs/2403.08763) · [O-LoRA](https://arxiv.org/abs/2310.14152) · [LLaMA Pro](https://arxiv.org/abs/2401.02415) · [Fine-tuning vs retrieval](https://arxiv.org/abs/2312.05934)
- *Test-time learning:* [Test-Time Training](https://arxiv.org/abs/1909.13231) · [TTT layers](https://arxiv.org/abs/2407.04620) · [TTT for ARC](https://arxiv.org/abs/2411.07279) · [the skeptical read](https://arxiv.org/abs/2607.00368)
- *Self-modifying:* [SEAL](https://arxiv.org/abs/2506.10943) · [Transformer²](https://arxiv.org/abs/2501.06252) · [Darwin Gödel Machine](https://arxiv.org/abs/2505.22954) · [a self-referential weight matrix](https://arxiv.org/abs/2202.05780)
- *Agent architectures:* [CoALA](https://arxiv.org/abs/2309.02427) · [ReAct](https://arxiv.org/abs/2210.03629) · [Reflexion](https://arxiv.org/abs/2303.11366) · [Voyager](https://arxiv.org/abs/2305.16291) · [harness engineering](https://lilianweng.github.io/posts/2026-07-04-harness/)
