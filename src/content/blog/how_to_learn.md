...
The pronunciation half, at least, I could make concrete — so I built [my-english](https://github.com/locchh/my-english), a tiny browser page for exactly that. It shows the full IPA chart — every English sound, color-coded into short vowels, long vowels, diphthongs, and voiced/unvoiced consonants, each with example words — and it converts any text you type straight into its IPA transcription, stress marks and all, right in the browser with no backend (espeak-ng compiled to WebAssembly does the work on your machine). Because I'm Vietnamese, every sound also carries a rough Vietnamese approximation, with a star on the ones — like the *th* in θ and ð — that don't exist in Vietnamese at all. It won't move your mouth for you; that's still the reading-aloud work below. But it answers the question that has to come first: *what is the correct sound I'm even aiming for?*
...
I did the same to learn how Copilot-style completion actually works under the hood: I built [whisperer](https://github.com/iteam1/whisperer), a small VSCode extension that turns the Claude Code CLI I already run into an inline-completion engine — you drive, it fills in the next few lines. Building it taught me things reading about it never would: how VSCode actually renders ghost text (it *pulls* from you, it doesn't let you push), how two processes talk over a tiny local server, why the simplest transport usually wins. And it stayed small on purpose — I kept a roadmap that rebuilds it from scratch phase by phase, and the design only clicked once I started *removing* pieces (an MCP server, a WebSocket layer, focus-based routing) until plain HTTP over a workspace path was all that was left.
...
These two are better together. Run Veronica's coaching armor alongside [whisperer](https://github.com/iteam1/whisperer) and you get the most hands-on setup I know of: Veronica makes you *understand* each step and write it yourself, and whisperer keeps you the driver — you type the code and pull a completion only when you want one, instead of watching an agent spit out the whole file while you sit there (that is the "someone else lifting your weights" trap from before). Your hands stay on the keyboard the entire session, and that is exactly what turns syntax into muscle memory. The reps only count if they are *yours*.
