...
- **Run your own WIP = 1, and batch to defend it.** The switch tax is real and it's twenty-three minutes. So don't interleave — *batch*. Review one agent's output to completion, then the next. Give the seat one thing at a time, the same discipline you give each subagent, because you are subject to the exact same law they are. Cal Newport's first principle of [slow productivity](https://www.youtube.com/watch?v=0HMjTxKRbaI) is this same rule from the other direction: *do fewer things* never meant *accomplish fewer things* — it means do fewer *at once*, so more of the day goes to completing commitments instead of servicing residue. Fewer at once finishes more.
- **Pace the seat — it runs on biology, not silicon.** The swarm redlines around the clock because its ceiling is hardware; yours is a nervous system. Newport calls the state that constant switching produces "exhausting and frustrating" — burnout isn't a character flaw, it's what happens when a biological seat tries to match a silicon duty cycle. His second principle, a *natural pace*, is the fix: busy days and quiet days, busy seasons and quiet seasons — humans worked in seasons for a hundred thousand years before knowledge work pretended otherwise. The swarm's uptime is not your benchmark. A seat that rests decides better than a seat that redlines, and deciding is the only thing the seat is for.
...
And the way we *measure* the work breaks at the same moment. Newport's [diagnosis](https://www.youtube.com/watch?v=0HMjTxKRbaI) is that knowledge work never had a real productivity metric — no Model-Ts-per-labor-hour — so we substituted **pseudo-productivity**: "visible activity as a crude proxy for useful effort." Look busy, answer fast, be seen active in the chat. An agent swarm is the reductio of that proxy: it produces unlimited visible activity — commits, PRs, messages, around the clock — none of it evidence of anything. The moment activity becomes free, activity becomes worthless as a signal, and the only measure left standing is the one [*Slow Productivity*](https://calnewport.com/) argues we should have been using all along: the quality of what actually ships over time. Which is precisely the seat's product, and nobody else's.
