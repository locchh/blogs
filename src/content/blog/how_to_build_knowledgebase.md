...
Now read that against the Hebb flaw from the top of the post. "Gets stronger with use" feeds on itself: strong links get retrieved more, which makes them stronger, which gets them retrieved more. Run that forever and everything connects strongly to everything — and a graph where everything is strong is useless, like a book where every sentence is highlighted. So **the decay-and-prune is not a speed tweak. It is the brake that stops the strengthening from destroying the graph.** A knowledge base that only ever calls `on_coactivation` and never `sleep` is a Hebbian network with the brakes cut — it just grows denser and dumber forever. (This is the same "a memory that only ever adds destroys itself" from the opening, now with a line number.)
