...
Notice the *shape*, because it is the part that is easy to get wrong: two branches off one input, not a three-step relay. The moment something comes in, working memory and the hippocampus receive it **at the same time**, as two separate recipients — one does not feed the other. Working memory flares and fades on its own; for lasting memory it is a **dead end**, handing nothing on. Only the hippocampus's copy is carried — during sleep — up into the neocortex. The software mapping inherits this exactly: one query lands in the **context window** and in **write-time storage** at once; the context window answers and is thrown away, while only the write-time copy is ever fed to the **sleep job**.
